...
var level;
var sounds;
// music object: wraps AudiusMusic with the same interface the game expects
var music;

var lastTime;
function init() {
  // Use Audius for background music; stub overworld/underground/clear/death
  music = {
    overworld: AudiusMusic.overworld,
    underground: AudiusMusic.underground,
    clear: { play: function(){}, pause: function(){}, currentTime: 0 },
    death: (function() {
      var a = new Audio('sounds/mariodie.wav');
      return {
        play: function() { a.currentTime = 0; a.play().catch(function(){}); },
  ...
  // Patch sound play to handle browser autoplay restrictions gracefully
  Object.keys(sounds).forEach(function(k) {
    var orig = sounds[k];
    sounds[k] = {
      get currentTime() { return orig.currentTime; },
      set currentTime(v) { orig.currentTime = v; },
      play: function() {
        orig.currentTime = 0;
        orig.play().catch(function(){});
      },
      pause: function() { orig.pause(); }
    };
  });
  ...
  // Start Audius music (non-blocking — will play once streams resolve)
  AudiusMusic.init();
